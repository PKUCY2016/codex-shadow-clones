...
3. 运行 scripts/preflight.py 核验 Codex 桌面应用。已核查版本为 com.openai.codex、26.917.71314、10954；后续 build 必须通过 shadow_compat.py 的签名、独立数据目录接口及无账号临时项目接口探测。失败时停止启动并报告原因，不删除兼容检查、不改应用签名、不假定新版兼容。
4. 默认源目录是 ~/.codex。如我使用其他原实例目录，先报告当前不支持的差异，不擅自改全局 HOME 或替换原账号。不要读取、打印、上传或要求我粘贴 auth.json、令牌、授权链接、私人会话。不要把 .runtime 内容提交到 Git。
5. 在仓库先运行预检，再运行离线测试；新 build 的预检仅在自动清理的临时目录中写入无账号数据，不读取现有账号或聊天：
...
不支持 Windows / Linux；桌面版本升级后先运行自动兼容探测，失败时仍需人工适配。
