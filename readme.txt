...
9. 更新目标分身的历史或配置前，必须确认该目标已退出。不要关闭有进行中任务的应用。配置以源快照为准；历史仅刷新未独立修改的导入记录，冲突保留目标版本。源当前未结束回合暂不复制。不要手动覆盖正在写入的 SQLite 数据库。
