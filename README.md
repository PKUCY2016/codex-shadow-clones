...
[本次更新](#本次更新v041) · [更新安装](#更新到最新版) · [快速开始](#快速开始) · [直接交给 Codex](readme.txt) · [使用与排障](docs/USAGE.md) · [参考项目](docs/REFERENCES.md) · [实现边界](DESIGN.md)

## 本次更新：v0.4.1

- **[新增] 删除分身**：退出目标后可从卡片删除；数据完整移入本地备份，支持按恢复记录找回。原实例受保护，编号不复用。
- **[新增] 新版提示**：启动后及每 6 小时自动检查公开版本信息，面板与菜单栏提示新版，并提供安装说明；可手动检查。
- **[改进] 菜单栏常驻**：只显示忍者头像，去掉“影”字；明确关窗口后常驻，打开忍者菜单后可按 ⌘Q 退出菜单栏。
- **[改进] 多源历史同步**：汇总所有实例已结束的任务，运行目标延后补齐，分叉保留独立副本。
- **[改进] 新分身基线**：使用短物理 home，复制本地记忆快照与暂停的自动化模板，避免 daemon 超长路径和定时任务重复执行。
- **[修复] 原实例入口**：面板点击 Codex 1 时，macOS 会打开或聚焦官方原 Codex，不再因进程探测暂时为空而报错；普通启动入口发现旧管理服务时会自动安全重载。
- **[工程] 更新与验证**：版本号统一维护在 `version.json`；更新启动器可重载管理服务和菜单栏，新增 macOS / Python 3.11、3.14 持续集成检查。

完整记录见 [CHANGELOG](CHANGELOG.md)。额度查询逻辑未因短暂网络失败做额外改动。
